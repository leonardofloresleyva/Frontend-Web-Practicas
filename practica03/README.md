...
No hizo falta ninguna base de datos real; solo se requirió un simple diccionario para almacenar los préstamos. Esto evidencia claramente la ventaja del desacoplamiento mediante el patrón Repository, ya que **el servicio desconoce la implementación concreta del repositorio**.
...
A lo mucho, se crearía un nuevo archivo con una clase que implemente el acceso a la base de datos y únicamente **se modificaría una línea de código** (cuando se instancia la clase en el archivo main.ts). Se modifica tan poco porque se sigue implementando la misma interfaz (el patrón sigue intacto), por lo que, **a ojos del servicio, nada ha cambiado**.
